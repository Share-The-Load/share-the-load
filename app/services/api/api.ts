...
export class Api {
  ...
  async getProfile(): Promise<{ kind: "ok"; profile: User } | GeneralApiProblem> {
    const response: ApiResponse<User> = await this.apisauce.get(`/profile`)
    if (!response.ok) {
      const problem = getGeneralApiProblem(response)
      if (problem) return problem
    }
  ...
  async updateLoadTime(loadTime: number): Promise<{ kind: "ok" } | GeneralApiProblem> {
    ...
    if (!response.ok) { const problem = getGeneralApiProblem(response); if (problem) return problem }
    return { kind: "ok" }
  }
  ...
  async deleteLoad(loadId: number): Promise<{ kind: "ok" } | GeneralApiProblem> {
    ...
    if (!response.ok) { const problem = getGeneralApiProblem(response); if (problem) return problem }
    if (response.data?.status === "error") return { kind: "bad-data" }
    return { kind: "ok" }
  }
